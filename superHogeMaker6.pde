...
PImage title, start, play, make;  //タイトルと各種ボタン
Block block;

//あそぶモード用変数
PImage goal;  //ゴールの旗
PImage FMS;
//boolean jump;  //キャラの動き管理
Player player;
  ...
  case 2:
    ...
    for (Item i : items) {
      if (i.isItem(player.posX+8*n, player.posY+8*n)) {
        ...
        //つぶやく
        tweet.add(new int[] {
          (int)random(-16, 48), (int)random(4, 24), 0
        }
        );
        bird.rewind();
        bird.play();
...
void keyPressed() {
  if (keyCode==RIGHT) {
    if (page==2 && (player.alive && !player.finish)) {
      player.right=true;
  ...
  if (keyCode==LEFT && page==2 && player.alive && !player.finish) {
    player.left=true;
    player.isFacingRight=false;
  }

  if (keyCode==UP && page==2 && player.alive && !player.finish) {
